				...
				value:
					this.state === "submit" || this.state === "cancel"
						? this.value
							? active
							: inactive
						: `${opt(!!this.value, active)} ${color.dim("/")} ${opt(
								!this.value,
								inactive
						  )}`,
...
export const groupMultiselect = <Value>(
	...
	const opt = (
		...
		return `${color.dim(prefix)}${color.dim(S.CHECKBOX_INACTIVE)} ${color.dim(
			label
		)}`;
	};
			...
			switch (this.state) {
				...
				case "error": {
					const footer = this.error
						...
						.map((ln, i) =>
							i === 0
								? `${color.yellow(S.BAR_END)}  ${color.yellow(ln)}`
								: `   ${ln}`
						)
						.join("\n");
...
export const log = {
	message: (
		message = "",
		{ symbol = color.gray(S.BAR) }: LogMessageOptions = {}
	) => {
		process.stdout.write(
			`${format(message, {
				firstLine: {
